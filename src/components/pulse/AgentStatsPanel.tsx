...
export default function AgentStatsPanel({ agents, stateCount }: Props) {
  ...
  return (
    <div className="bg-white/60 backdrop-blur-md rounded-xl border border-black/8 p-4 w-[200px] shadow-sm">
      <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 font-display">Agent Stats</h3>

      {/* State distribution */}
      <div className="space-y-1 mb-3">
        <span className="text-[9px] text-gray-400 uppercase tracking-wider">Activity</span>
        {Object.entries(stateCount).map(([state, count]) => (
          <div key={state} className="flex items-center gap-1.5 text-[10px]">
            ...
            <span className="text-gray-500 w-6 text-right font-mono">{count}</span>
          </div>
        ))}
      </div>
