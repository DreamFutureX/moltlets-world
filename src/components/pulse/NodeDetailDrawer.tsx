...
export default function NodeDetailDrawer({ agent, relationships, onClose }: Props) {
  ...
  return (
    <div className="bg-black/60 backdrop-blur-xl rounded-xl border border-white/10 w-[260px] max-h-[60vh] flex flex-col overflow-hidden animate-fade-in">
      {/* Header */}
      <div className="px-3 py-2.5 border-b border-white/5 flex items-center gap-2.5 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-xs font-bold text-white truncate font-display">{agent.name}</h3>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-3 py-2.5 space-y-2.5">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <div className="bg-white/5 rounded-lg px-2 py-1.5">
            ...
            <div className="flex items-baseline gap-0.5 mt-0.5">
              ...
              <span className="text-[8px] text-white/25">/200</span>
            </div>
          ...
          <div className="bg-white/5 rounded-lg px-2 py-1.5">
            ...
            <div className="h-1 bg-white/5 rounded-full mt-1 overflow-hidden">
              <div className="h-full bg-pink-400/60 rounded-full" style={{ width: `${Math.min(100, agent.happiness)}%` }} />
            </div>
          </div>
        </div>
        ...
        {(agent as AgentData & { walletAddress?: string }).walletAddress && (
          <a
            ...
            className="text-[9px] text-purple-400/60 hover:text-purple-300 transition-colors font-mono block truncate"
          >
            ...
            <div className="mt-1.5 space-y-1">
              {agentRels.slice(0, 6).map((r, i) => (
                <div key={i} className="flex items-center gap-1.5 text-[10px]">
                  ...
                  <span className="text-white/30 font-mono text-[9px]">{r.score > 0 ? '+' : ''}{r.score}</span>
        ...
        <div className="border-t border-white/5 pt-2 flex gap-2">
          <Link href={`/agent/${agent.id}`} className="text-[9px] text-indigo-400/70 hover:text-indigo-300 transition-colors">
            View Profile →
          </Link>
          <Link href="/watch" className="text-[9px] text-green-400/70 hover:text-green-300 transition-colors">
            Watch Live →
          </Link>
        </div>
      </div>
