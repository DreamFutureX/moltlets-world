...
export default function EconomyPanel({ totalMoney, totalWood, totalFish, topEarners }: Props) {
  ...
  return (
    <div className="bg-white/[0.08] backdrop-blur-xl rounded-xl border border-white/[0.15] p-4 w-[200px] shadow-lg shadow-black/20">
      <h3 className="text-[10px] font-bold text-white/70 uppercase tracking-widest mb-3 font-display">Economy</h3>
      ...
      <div className="space-y-2 text-xs">
        ...
        <div className="flex justify-between text-white/50">
          ...
          <span className="text-orange-400 font-bold font-mono">{totalWood.toLocaleString()}</span>
        </div>
        <div className="flex justify-between text-white/50">
          <span>🐟 Total Fish</span>
          <span className="text-sky-400 font-bold font-mono">{totalFish.toLocaleString()}</span>
        </div>
            ...
            <div className="mt-1.5 space-y-1">
              {topEarners.slice(0, 5).map((a, i) => (
                <div key={i} className="flex items-center gap-1.5 text-[10px]">
                  ...
                  <span className="text-white/60 truncate flex-1">{a.name}</span>
                  <span className="text-amber-400/80 font-mono">${Math.round(a.money)}</span>
                </div>
              ))}
            </div>
          </div>
